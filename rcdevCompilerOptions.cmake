...
if( UNIX )
	...
	# Map both the repo-specific configuration names and the common Debug/Release
	# so external projects (which may use Debug/Release) inherit the static CRT.
	set(CMAKE_MSVC_RUNTIME_LIBRARY "$<$<CONFIG:Debug_Mdd>:MultiThreadedDebug>$<$<CONFIG:Debug>:MultiThreadedDebug>$<$<CONFIG:Release_MD>:MultiThreaded>$<$<CONFIG:Release>:MultiThreaded>" CACHE STRING "" FORCE)
	
	# For external projects: resolve CMAKE_MSVC_RUNTIME_LIBRARY to a concrete value
	# based on CMAKE_BUILD_TYPE or CMAKE_CONFIGURATION_TYPES (if set at config time)
	if(CMAKE_BUILD_TYPE)
		if(CMAKE_BUILD_TYPE STREQUAL "Debug_Mdd" OR CMAKE_BUILD_TYPE STREQUAL "Debug")
			set(CMAKE_MSVC_RUNTIME_LIBRARY_FOR_EXTERNAL "MultiThreadedDebug")
		else()
			set(CMAKE_MSVC_RUNTIME_LIBRARY_FOR_EXTERNAL "MultiThreaded")
		endif()
	else()
		# Multi-config generator: default to Release/MultiThreaded unless we detect Debug
		set(CMAKE_MSVC_RUNTIME_LIBRARY_FOR_EXTERNAL "MultiThreaded")
	endif()
	...
	set(CMAKE_C_FLAGS_DEBUG_MDD "${RCDEV_C_FLAGS_INIT} /Gy- /Od /RTC1 /MTd /GF- ${_pdb_compile_flags} " CACHE STRING "" FORCE)
	set(CMAKE_C_FLAGS_RELEASE_MD "${RCDEV_C_FLAGS_INIT} /O2 /Ob2 /Ot /MT /Gy /GF /D NDEBUG ${_pdb_compile_flags} " CACHE STRING "" FORCE)

	# Also provide the standard Debug/Release variables so external/third-party
	# CMake projects that use the conventional config names get /MT(/MTd).
	set(CMAKE_C_FLAGS_DEBUG "${RCDEV_C_FLAGS_INIT} /Gy- /Od /RTC1 /MTd /GF- ${_pdb_compile_flags} " CACHE STRING "" FORCE)
	set(CMAKE_C_FLAGS_RELEASE "${RCDEV_C_FLAGS_INIT} /O2 /Ob2 /Ot /MT /Gy /GF /D NDEBUG ${_pdb_compile_flags} " CACHE STRING "" FORCE)
	...
	set(CMAKE_CXX_FLAGS_DEBUG_MDD "${RCDEV_CXX_FLAGS_INIT} /Gy- /Od /RTC1 /MTd /GF-" CACHE STRING "" FORCE)
	set(CMAKE_CXX_FLAGS_RELEASE_MD "${RCDEV_CXX_FLAGS_INIT} /O2 /Ob2 /Ot /MT /Gy /GF /D NDEBUG" CACHE STRING "" FORCE)

	# Provide conventional Debug/Release variants for external projects.
	set(CMAKE_CXX_FLAGS_DEBUG "${RCDEV_CXX_FLAGS_INIT} /Gy- /Od /RTC1 /MTd /GF-" CACHE STRING "" FORCE)
	set(CMAKE_CXX_FLAGS_RELEASE "${RCDEV_CXX_FLAGS_INIT} /O2 /Ob2 /Ot /MT /Gy /GF /D NDEBUG" CACHE STRING "" FORCE)
	...
	set(CMAKE_SHARED_LINKER_FLAGS_DEBUG_MDD "${CMAKE_SHARED_LINKER_FLAGS_INIT} /DEBUG /NODEFAULTLIB:MSVCRTD /INCREMENTAL:NO"  CACHE STRING "" FORCE)
	...
	set(CMAKE_EXE_LINKER_FLAGS_DEBUG_MDD "${CMAKE_EXE_LINKER_FLAGS_INIT} /DEBUG /NODEFAULTLIB:MSVCRTD /INCREMENTAL:NO"  CACHE STRING "")
